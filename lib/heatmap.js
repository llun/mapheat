...
class Heatmap {
  ...
  draw(block) {
    let size = this.options.size
      , pane = this._createCanvas(size, size)
    ...
    let cropSize = size / 3
      , crop = this._createCanvas(cropSize, cropSize)
  ...
  write(directory, callback) {
    return new Promise((resolve, reject) => {
      ...
      let blocks = Object.keys(this._blocks).filter(block => (this._blocks[block].points.size > 0))
      let promises = blocks.map(block => {
        return new Promise((resolve, reject) => {
          let path = `${directory}/${block}.png`
          fs.writeFile(path, this.draw(this._blocks[block]).toBuffer(), error => {
            if (error) return reject(error)
            resolve(path)
          })
        })
